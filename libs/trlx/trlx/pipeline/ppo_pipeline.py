...
from trlx.data.ppo_types import PPORLBatch, VLMPPORLBatch, PPORLElement
from trlx.pipeline import BaseRolloutStore


def ppo_collate_fn(
    padding_side: str, pad_token_id: int, elems: Iterable[PPORLElement], use_vlm=False
):
    ...
    image_emb_tensors = torch.stack([elem.image_emb_tensor for elem in elems], dim=0)
    return VLMPPORLBatch(
        image_emb_tensors,
        query_tensors,
        # Right pad the rest, to have a single horizontal query/response split
        pad_sequence(
...
class PPORolloutStorage(BaseRolloutStore):
    ...
    def __init__(self, pad_token_id, padding_side, use_vlm=False):
        super().__init__()
    ...
    def create_loader(
        ...
        collate_fn = partial(
            ppo_collate_fn, self.padding_side, self.pad_token_id, use_vlm=self.use_vlm
        )
