...
def chunks(lst, chunk_size):
    """Yields successive chunk size chunks from lst."""
    for i in range(0, len(lst), chunk_size):
        yield lst[i : i + chunk_size]


@register_trainer
class AccelerateVLMRLTrainer(BaseRLTrainer):
    """
    ...
    def __init__(self, config, **kwargs):  # noqa: C901
        ...
        ipg_handler = InitProcessGroupKwargs(timeout=timedelta(minutes=60))
        ...
        if "vicuna" in config.tokenizer.tokenizer_path:
        ...
        if hasattr(config.model, "ref_to_model"):
            if isinstance(config.model, object):
                model_name = config.model.ref_to_model.__class__.__name__
            else:
                model_name = config.model.ref_to_model.__name__
        elif not isinstance(config.model.model_path, str):
    ...
    def setup_model(self):
        ...
        if hasattr(self.config.model, "ref_to_model"):
            if isinstance(self.config.model, object):  # if model is an instance of any class
                return self.config.model.ref_to_model
            return self.config.model.ref_to_model(**self.config.model.model_init_kwargs)
    ...
    def generate_eval(self, image_emb, input_ids, attention_mask=None, **kwargs):
        """Wraps hf's `generate` adding some specific method's defaults"""
        input_ids = input_ids.to(self.accelerator.device)
        if attention_mask is not None:
            attention_mask = attention_mask.to(self.accelerator.device)
    ...
    def load(self, directory: Optional[str] = None, **kwargs):
        ...
        print(f"Load checkpoint from {directory or self.config.train.checkpoint_dir}")
    ...
    def evaluate(self):  # noqa: C901
        ...
        for i_sweep, gen_sweep_value in enumerate(gen_sweep_values):
            ...
            for i_prompt, prompts in enumerate(self.eval_dataloader):
                ...
                #     if k not in ["image", "image_emb", "input_ids", "attention_mask"]
                # }
                ...
                image_tensors = self.accelerator.gather_for_metrics(prompts["image"])

                all_images.append(image_tensors)
                all_samples.extend(samples.tolist())
                all_text_prompts.extend(text_prompts.tolist())
            ...
            if self.accelerator.is_main_process:
                str_samples, str_prompts, str_outputs = self.decode(all_text_prompts, all_samples, all_prompt_sizes)
                ...
                if self.reward_fn:
                    ...
                    rewards, reward_dict = self.reward_fn(
                        images=all_images,
                        samples=str_samples,
                        prompts=str_prompts,
                        outputs=str_outputs,
                    ...
                    #         [reward.sum().item() for reward in rewards], dtype=float
                    #     )
                    # elif isinstance(rewards[0], list):
                    #     rewards = torch.tensor(
                    #         [sum(reward) for reward in rewards], dtype=float
                    #     )
                    # else:
                    ...
                    rec_reward = reward_dict["rec_reward"]
                    rec_reward = torch.tensor([sum(reward) for reward in rec_reward], dtype=float)
                    mean_rec_reward = rec_reward.mean().item()
                    columns.append("rec_reward")
                    if not isinstance(rec_reward, list):
                        rec_reward = rec_reward.tolist()
                    columns_data.append(rec_reward)
                    stats[f"reward/rec_reward_mean{sweep_suffix}"] = mean_rec_reward

                    obj_penalty = reward_dict["obj_penalty"]
                    obj_penalty = torch.tensor([sum(penalty) for penalty in obj_penalty], dtype=float)
                    mean_obj_penalty = obj_penalty.mean().item()
                    columns.append("obj_penalty")
                    if not isinstance(obj_penalty, list):
                        obj_penalty = obj_penalty.tolist()
                    columns_data.append(obj_penalty)
                    stats[f"reward/obj_penalty_mean{sweep_suffix}"] = mean_obj_penalty

                    att_penalty = reward_dict["att_penalty"]
                    att_penalty = torch.tensor([sum(penalty) for penalty in att_penalty], dtype=float)
                    mean_att_penalty = att_penalty.mean().item()
                    columns.append("att_penalty")
                    if not isinstance(att_penalty, list):
                        att_penalty = att_penalty.tolist()
                    columns_data.append(att_penalty)
                    stats[f"reward/att_penalty_mean{sweep_suffix}"] = mean_att_penalty

                    rel_penalty = reward_dict["rel_penalty"]
                    rel_penalty = torch.tensor([sum(penalty) for penalty in rel_penalty], dtype=float)
                    mean_rel_penalty = rel_penalty.mean().item()
                    columns.append("rel_penalty")
                    if not isinstance(rel_penalty, list):
                        rel_penalty = rel_penalty.tolist()
                    columns_data.append(rel_penalty)
                    stats[f"reward/rel_penalty_mean{sweep_suffix}"] = mean_rel_penalty

                    pos_penalty = reward_dict["pos_penalty"]
                    pos_penalty = torch.tensor([sum(penalty) for penalty in pos_penalty], dtype=float)
                    mean_pos_penalty = pos_penalty.mean().item()
                    columns.append("pos_penalty")
                    if not isinstance(pos_penalty, list):
                        pos_penalty = pos_penalty.tolist()
                    columns_data.append(pos_penalty)
                    stats[f"reward/pos_penalty_mean{sweep_suffix}"] = mean_pos_penalty
