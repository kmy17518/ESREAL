...
from trlx.data.ppo_types import VLMPPORLBatch, VLMPPORLElement
from trlx.models.modeling_ppo import (
    AdaptiveKLController,
    FixedKLController,
)
from trlx.pipeline.offline_pipeline import VLMDatasetPipeline
from trlx.pipeline.ppo_pipeline import PPORolloutStorage
from trlx.trainer import register_trainer
from trlx.trainer.accelerate_vlm_base_trainer import AccelerateVLMRLTrainer
...
@register_trainer
class AccelerateVLMPPOTrainer(AccelerateVLMRLTrainer):
    """PPO Accelerate Trainer"""

    reward_fn: Callable[[List[str], List[Dict[str, Any]], List[str]], List[float]]
    ...
    def __init__(self, config: TRLConfig, **kwargs):
        ...
        self.store = PPORolloutStorage(self.tokenizer.pad_token_id, self.tokenizer.padding_side, use_vlm=True)
        ...
        if hasattr(self.config.model, "ref_to_model"):
            if self.config.model.model_copy is not None:
                self.ref_model = self.config.model.model_copy
            else:
                self.ref_model = self.config.model.ref_to_model(**self.config.model.model_init_kwargs)
        elif not hasattr(self.model, "frozen_head") and not self.model.peft_type:
    ...
    def loss(self, batch: VLMPPORLBatch):
        """Forward pass & loss
        ...
        image_emb_tensors = batch.image_emb_tensors.to(self.accelerator.device)
        ...
        if self.config.model.model_arch_type == "seq2seq":
            ...
            outputs = self.model(
                image_emb=image_emb_tensors,
                prompt_input_ids=query_tensors,
                input_ids=tokens,
                attention_mask=attention_mask,
    ...
    def add_prompt_pipeline(self, pipeline: VLMDatasetPipeline):
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(ppo_rl_elements) < num_rollouts:
            ...
            samples = self.generate(batch["image_emb"], batch["input_ids"], batch["attention_mask"])

            stats["time/rollout_generate"] = time() - rollout_generate_time

            image_tensors = batch.image
            image_emb_tensors = batch.image_emb
            text_prompt_tensors = batch.input_ids
            device = samples.device

            prompt_sizes = torch.tensor([text_prompt_tensors.shape[1]] * len(text_prompt_tensors), device=device)
            padded_samples = self.accelerator.pad_across_processes(
                samples, dim=1, pad_index=self.tokenizer.eos_token_id, pad_first=False
            )
            padded_prompts = self.accelerator.pad_across_processes(
                text_prompt_tensors, dim=1, pad_index=self.tokenizer.eos_token_id, pad_first=False
            )
            gathered_image_tensors = self.accelerator.gather(image_tensors)
            gathered_samples = self.accelerator.gather(padded_samples)
            gathered_prompts = self.accelerator.gather(padded_prompts)
            gathered_prompt_sizes = self.accelerator.gather(prompt_sizes)
            metadata = gather_dict(
                {k: v for k, v in batch.items() if k not in ["input_ids", "attention_mask", "image", "image_emb"]}
            )
            ...
            if self.accelerator.is_main_process:
                ...
                print("[*] reward_fn is called")
                all_scores, _ = self.reward_fn(
                    images=gathered_image_tensors,
            ...
            str_samples, str_prompts, str_outputs = self.decode(text_prompt_tensors, samples, append_eos_token=False)
            ...
            if self.config.model.model_arch_type == "seq2seq":
                ...
                with torch.no_grad():
                    ...
                    if hasattr(self.config.model, "ref_to_model"):  # TODO (alan): support hydra head
                    ...
                    elif hasattr(self.model, "frozen_head") or self.model.peft_type:
                ...
                all_tokens = torch.cat((text_prompt_tensors.to(device), sample_outputs), dim=1)
                ...
                with torch.no_grad():
                    logits, values = self.model(
                        image_emb=image_emb_tensors,
                        prompt_input_ids=text_prompt_tensors.to(device),
                        input_ids=all_tokens,
                        attention_mask=attention_mask,
                        position_ids=position_ids,
                    )
                    if hasattr(self.config.model, "ref_to_model"):  # TODO: support hydra head
                        ref_logits = self.ref_model(
                            image_emb=image_emb_tensors,
                            prompt_input_ids=text_prompt_tensors.to(device),
                            input_ids=all_tokens,
            ...
            if self.config.model.model_arch_type == "seq2seq":
                ...
                start = text_prompt_tensors.shape[1] - 1
            ...
            text_prompt_tensors = text_prompt_tensors.cpu()
            image_emb_tensors = image_emb_tensors.cpu()
            ...
            for sample_idx in range(n_samples):
                ...
                ppo_rl_elements.append(
                    VLMPPORLElement(
                        image_emb_tensor=image_emb_tensors[sample_idx],
                        query_tensor=text_prompt_tensors[sample_idx],
