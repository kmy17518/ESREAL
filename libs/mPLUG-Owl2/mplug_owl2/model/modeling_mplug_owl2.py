...
class MPLUGOwl2MetaForCausalLM(ABC):
    ...
    def encode_images(self, images, use_ref_visual_abstractor=False):
        image_features = self.get_model().vision_model(images).last_hidden_state

        if use_ref_visual_abstractor:
            image_features = self.get_model().ref_visual_abstractor(encoder_hidden_states=image_features).last_hidden_state
        else:
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = self.encode_images(images, use_ref_visual_abstractor=get_ref_logits)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            input_ids, modality_indicators, attention_mask, past_key_values, inputs_embeds, labels = \
                self.prepare_inputs_labels_for_multimodal(input_ids, attention_mask, past_key_values, labels, images, get_ref_logits=get_ref_logits)
