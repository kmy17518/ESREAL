...
def create_eva_vit_g(
    ...
    with time_measure("create model"):
    ...
    with time_measure("download model"):
        url = "https://storage.googleapis.com/sfr-vision-language-research/LAVIS/models/BLIP2/eva_vit_g.pth"
        cached_file = download_cached_file(url, check_hash=False, progress=True)
    with time_measure("load state_dict"):
        state_dict = torch.load(cached_file, map_location=device)
    with time_measure("interp_pos_embed"):
        interpolate_pos_embed(model, state_dict)

    with time_measure("load_state_dict"):
        incompatible_keys = model.load_state_dict(state_dict, strict=False)
    #     print(incompatible_keys)

    if precision == "fp16":
        #         model.to("cuda")
        with time_measure("convert_weights_to_fp16"):
